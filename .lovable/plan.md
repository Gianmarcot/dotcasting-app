...
- `src/components/profile/ImageCropModal.tsx`: aggiunta prop `ratios` (default `["2:3"]`), tab per proporzione mostrate solo se `ratios.length > 1`, stato di crop per ratio, callback `onSave(results: { ratio, blob, rect }[])`. `PhotoGalleryModal` passa `["2:3","1:1"]` solo per la prima foto principale. Rimane compatibile con l'uso attuale in `MediaGallerySection.tsx`.
