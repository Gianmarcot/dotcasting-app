...
- `src/components/profile/v2/DocumentsCard.tsx`: aggiungere `<SectionDivider />` prima di `UploadBlock`; rimuovere `error`/`warning` inline dal `FloatingInput` del CF (mantenendo lo stato rosso del bordo solo per l'errore formale) e rendere l'avviso come secondo elemento dello stesso `FieldGrid cols={2}` che contiene il campo, così da affiancarlo a destra.
