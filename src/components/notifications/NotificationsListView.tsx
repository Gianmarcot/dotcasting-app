...
const NotificationRow = ({
  ...
  return (
    <Link
      to={`${basePath}/${notification.id}`}
      className={cn(
        "flex gap-4 p-4 rounded-2xl transition-colors border border-transparent hover:bg-muted/30 hover:border-border",
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-3">
          ...
          {isUnread && <span className="shrink-0 w-2 h-2 bg-primary rounded-full mt-1.5" />}
        </div>
        {description && (
...
export const NotificationsListView = ({ basePath }: { basePath: string }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-up max-w-3xl">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="font-display uppercase tracking-wide text-2xl text-foreground">
            Notifiche
          </h1>
      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          {GROUP_ORDER.filter((g) => grouped[g]?.length).map((group) => (
            <section key={group}>
              <h2 className="font-display uppercase tracking-widest text-xs text-muted-foreground mb-3">
                {group}
              </h2>
              <div className="space-y-2">
                {grouped[group].map((n) => (
                  <NotificationRow key={n.id} notification={n} basePath={basePath} />
                ))}
              </div>
            </section>
          ))}
